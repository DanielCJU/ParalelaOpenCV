...
char* tiempoActual(char* linea){
    std::time_t datos;
    std::tm* info;
    std::time(&datos);
    info = std::localtime(&datos);

    std::strftime(linea,80,"%Y%m%d%H%M%S",info);
...
void matsnd(const Mat& m,int dest){
      ...
      int bytes=N.rows*N.cols*channels*bytespersample;
	  
      if(!N.isContinuous())
      { 
         N = m.clone();
      }
      memcpy(&buffer[3*sizeof(int)],N.data,bytes);
